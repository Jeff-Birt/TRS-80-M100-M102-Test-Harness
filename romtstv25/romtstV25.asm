...
;
;  Uses parts of the M100_dis_2013 data.
;
...
;   - Write into the first 256 bytes make sure the addresses are
;	  uniquely accessed. 
...
;Check the contents of the tested RAM		
		LXI H,ram3	; Standard RAM location 0A000H		
		MVI A,8
...
ramfail:
		ANI 7          ; Get the row that failed
		ADI '0'        ; Set as character
...
;Insert OPTROM Detect Code into selected RAM Block	
		...
		MVI B,25        ; Number of bytes to move
...
piotestdone:	
		...
		JMP lcdtest     ; Test the M100/102 main LCD
...
; Print the to the PIO A register		
		XRA A
		CALL prthex
...
lcdtest:
		...
		LXI H,lcdstr    ; Output diag LCD text
		CALL putstr
...
;RTS Set 0
 		MVI A,0C6H     ; Set Cursor to back to slot
		CALL putins	
...
;DTR Set 0
 		MVI A,0CEH     ; Set Cursor to back to start
		CALL putins	
...
;  CLK bit is tested in Clock tes
...
; Range 0040-0048H are read to RAM. If the switch works 
